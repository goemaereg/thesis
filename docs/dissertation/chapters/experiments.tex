...
As explained in section \ref{sec:method_localization_improvement}, we use the \acrshort{gt}-based bounding boxes mask method as an oracle, i.e. for providing upper bounds on the performance that could be obtained when iterative localization would be guided by the ground truth. Fig. \ref{fig:prec_iter_resnet50_syn_d1b} illustrates localization results for the d1b dataset.
...
The iterative localization metrics for the d4b dataset are shown in Fig. \ref{fig:prec_iter_resnet50_syn_d4b}. As for the d3b dataset, recall can be improved at the cost of precision for all localization methods. The 'add' merge strategy with stop threshold 1.0 provides the highest gain in recall at the cost of substantially decreasing precision.
...
Iterative localization results for the d2b dataset are shown in Fig. \ref{fig:prec_iter_mask_cam_resnet50_syn_d2b}. Where both localization precision and recall could be improved in oracle-based iterative localization, we don't see the same trend for score map based iterative localization. The 'add' merge strategy with iteration drop criterion 1.0 provides the highest recall improvements, albeit marginal, at the cost of decreased precision.
...
The iterative localization metrics for the d4b dataset are shown in Fig. \ref{fig:prec_iter_mask_cam_resnet50_syn_d4b}. As for the d3b dataset, recall can be improved at the cost of precision for all localization methods. The 'add' merge strategy with stop threshold 1.0 provides the highest gain in recall at the cost of substantially decreasing precision.
...
We evaluate the oracle-based iterative localization for the ImageNet dataset. Due to time constraints, we were unable to evaluate score map based iterative localization on ImageNet.
