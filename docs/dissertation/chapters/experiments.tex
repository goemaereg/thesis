...
As illustrated in Table \ref{tab:iterative_localizaton_parameters}, the number parameter combinations for iterative localization experiments is too large to draw conclusions from in a single table overview. 
...
\begin{itemize}
    \item Per dataset, network and localization method, we plot a two-dimensional plot with precision on the  vertical axis and recall on the horizontal axis for the different mask strategies, merge strategies and stop criteria.
    ...
    \item The horizontal and vertical lines of the baseline create four quadrants which allow easy graphical interpretation of iterative localization results, depending in which quadrant a result is plotted.
    \begin{itemize}
...
Here we evaluate the results of the iterative localization for the VGG16-GAP network on the synthetic test datasets. We only tests the datasets that have images with a background to evaluate the effect of using different bounding box mask strategies. The localization methods used are CAM, Grad-CAM++, Score-CAM and MinMaxCAM. The former three methods are evaluated on a model trained without regularization while the latter is evaluated on a model trained with MinMaxCAM regularization.
...
For all methods we observe that precision is worse than the baseline as no result exists above the baseline precision line. Recall is improved by the iterative localization for CAM, Grad-CAM++ and Score-CAM when using the drop or add merge strategy. The 'add' strategy comes at a greater precision cost, as this strategy will create more false positives. The 'drop' strategy improves recall at the smallest precision cost when the classification score drop threshold 0.25 or 0.5 is used. As threshold 1.0 actually means that there is no stop criterion, it's easy to see that this threshold has the worst precision for any mask or merge strategy. The small gain in recall is to be expected for a dataset with single-instance object images where the non-iterative approach already has a high localization performance.

Iterative localization results for the d2b dataset are shown in Fig. \ref{fig:prec_iter_vgg16_gap_syn_d2b}. Looking at the upper right quadrant, we can observe that there are iterative localization methods that improve both precision and recall. The iterative methods applying the mean mask strategy, unify merge strategy and a stop criterion with classification drop threshold of 0.25 or 0.5 provide the best precision and recall improvements. 

The add merge strategy combined without iteration drop criterion (i.e. threshold `1.0) gives the highest recall improvement but at the cost of severe precision drop. The worst performing approach is the unify merge strategy without drop criterion. Intuitively, masking images with bounding boxes found in previous iterations, and unifying newly predicted bounding boxes with overlapping boxes of previous iterations, increases the risk of no longer matching predicted bounding boxes with ground truth boxes, and thus impacting the recall.
...
The iterative localization results for the d3b dataset are illustrated in Fig. \ref{fig:prec_iter_vgg16_gap_syn_d3b}. We see that the iterative approach can improve both precision and recall for the CAM and Grad-CAM++ method when applying the mean mask strategy combined with unify merge strategy and stop criterion threshold of 0.25 or 0.5. These combinations also improved both recall and precision for the d2b dataset. For MinMaxCAM and Score-CAM, recall can be improved, but always at the cost of decreased precision.
...
The iterative localization metrics for the d4b dataset are shown in Fig. \ref{fig:prec_iter_vgg16_gap_syn_d4b}. Grad-CAM++ is the only method where both precision and recall can be improved by applying the drop or unify merge strategy combined with zero mask strategy and a classification score drop threshold of 0.25. All other combinations improve recall at the cost of decreasing the precision.
...
The iterative localization methods for the ResNet-50 network evaluated on the ImageNet validation dataset, are shown in Fig. \ref{fig:prec_iter_resnet50_imagenet}. The best performing iterative localization methods are those that are positioned closest to the upper right corner of the plot. Observing the plots for all localization methods, we can see two distinct groups of iterative approaches: One group that improves recall at the cost of decreasing precision, and another group that performs worse for both precision and recall when compared to the baseline.
