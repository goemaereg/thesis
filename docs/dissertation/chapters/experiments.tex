...
\bottomrule
\end{tabular}
\caption[MaxBoxAccV3 recall for VGG16-GAP on synthetic datasets]{MaxBoxAccV3 recall for VGG16-GAP on synthetic datasets. First 3 rows show results for CAM-based methods evaluated on the non-regularized model. Last 3 rows show results for CAM-based models evaluated on MinMaxCAM-regularized model. Column-wise minimum and maximum per sub-table are highlighted in red and green. Global column-wise minimum and maximum are underlined.}
...
\bottomrule
\end{tabular}
\caption[MaxBoxAccV3 precision for VGG16-GAP on synthetic datasets]{MaxBoxAccV3 precision for VGG16-GAP on synthetic datasets. First 3 rows show results for CAM-based methods evaluated on the non-regularized model. Last 3 rows show results for CAM-based models evaluated on MinMaxCAM-regularized model. Column-wise minimum and maximum per sub-table are highlighted in red and green. Global column-wise minimum and maximum are underlined.}
...
\bottomrule
\end{tabular}
\caption[PxAP for VGG16-GAP on synthetic datasets]{PxAP for VGG16-GAP on synthetic datasets. First 3 rows show results for CAM-based methods evaluated on the non-regularized model. Last 3 rows show results for CAM-based models evaluated on MinMaxCAM-regularized model. Column-wise minimum and maximum per sub-table are highlighted in red and green. Global column-wise minimum and maximum are underlined.}
...
\begin{figure}[h]
    ...
    \caption[Classification accuracy and MaxBoxAccV3 recall on non-regularized VGG16-GAP for synthetic datasets]{classification accuracy and MaxBoxAccV3 recall on non-regularized VGG16-GAP for synthetic datasets, evaluated using CAM method.}
    \caption*{Source: Author}
    \label{fig:loc_vs_acc_vgg16_gap_cam_synthetic}
    \end{center}
\end{figure}

Figure \ref{fig:loc_vs_acc_vgg16_gap_minmaxcam_synthetic} shows training classification and CAM MaxBoxAccV3 recall for the model trained with MinMaxCAM regularization on the different datasets. Here, we observe that, except for single instance datasets, models trained for the t-variant datasets show a curve with decreasing MaxBoxAccV3 recall at later training epochs. This is in line with results in Table \ref{tab:maxboxaccv3_recall_vgg16_gap_synthetic}. As Wang \textit{et al.} \cite{wang2021minmaxcam} set out, the condition for common region regularization to work, is that a set of images for the same class should have different backgrounds. This clearly is not the case for the t-variant datasets.

\begin{figure}[ht]
    \begin{center}       
    \includegraphics[width=\textwidth]{fig_loc_vs_acc_vgg16_gap_minmaxcam_synthetic.png}
    \caption[Classification accuracy and MaxBoxAccV3 recall on MinMaxCAM-regularized VGG16-GAP for synthetic datasets]{Classification accuracy and MaxBoxAccV3 recall on MinMaxCAM-regularized VGG16-GAP for synthetic datasets, evaluated using CAM method.}
    \caption*{Source: Author}
...
To prove our indication that Grad-CAM++ activates on larger areas, we measured the fraction of activated pixels and the average of normalized pixel values of each score map computed using the CAM-based methods for VGG16 on the synthetic datasets. We averaged both metrics for all score maps, respectively giving us the average score map activations and average score map energy. Both metrics enable us to quantitatively compare the average size and intensity of activated areas in the score map for the different CAM-based methods.
...
\bottomrule
\end{tabular}
\caption[MaxBoxAccV3 recall for ResNet-50 on synthetic datasets]{MaxBoxAccV3 recall for ResNet-50 on synthetic datasets. First 3 rows show results for CAM-based methods evaluated on the non-regularized model. Last 3 rows show results for CAM-based models evaluated on MinMaxCAM-regularized model.  Column-wise minimum and maximum per sub-table are highlighted in red and green. Global column-wise minimum and maximum are underlined.}
...
\bottomrule
\end{tabular}
\caption[MaxBoxAccV3 precision for ResNet-50 on synthetic datasets]{MaxBoxAccV3 precision for ResNet-50 on synthetic datasets. First 3 rows show results for CAM-based methods evaluated on the non-regularized model. Last 3 rows show results for CAM-based models evaluated on MinMaxCAM-regularized model. Column-wise minimum and maximum per sub-table are highlighted in red and green. Global column-wise minimum and maximum are underlined.}
...
\bottomrule
\end{tabular}
\caption[PxAP for ResNet-50 on synthetic datasets]{PxAP for ResNet-50 on synthetic datasets. First 3 rows show results for CAM-based methods evaluated on the non-regularized model. Last 3 rows show results for CAM-based models evaluated on MinMaxCAM-regularized model. Column-wise minimum and maximum per sub-table are highlighted in red and green. Global column-wise minimum and maximum are underlined.}
...
\bottomrule
\end{tabular}
\caption[Localization metrics for ResNet-50 on ImageNet]{Localization metrics for ResNet-50 on ImageNet. First 3 rows show results for CAM-based methods evaluated on the non-regularized model. Last 3 rows show results for CAM-based models evaluated on MinMaxCAM-regularized model. Column-wise minimum and maximum per sub-table are highlighted in red and green. Global column-wise minimum and maximum are underlined.}
...
\begin{figure}[h]
    ...
    \caption[CAM MaxBoxAccV3 recall on ResNet-50 for ImageNet split by image object instance count]{CAM MaxBoxAccV3 recall on ResNet-50 for ImageNet split by image object instance count.}
    \caption*{Source: Author}
    \label{fig:resnet50_imagenet_inst_recall}
    \end{center}
\end{figure}

We observe that recall performance depends on the number of object instances in the images. Recall is highest for the single-instance dataset, and significantly drops significantly as more object instances are present in the images of a dataset. For ImageNet datasets with eight or more instances, recall seems to be rather similar. We observed a similar pattern for the synthetic datasets: Recall decreases as images in datasets have more object instances per image.
...
\begin{figure}[h]
    ...
    \caption[CAM MaxBoxAccV3 precision on ResNet-50 for ImageNet split by image object instance count]{CAM MaxBoxAccV3 precision on ResNet-50 for ImageNet split by image object instance count.}
    \caption*{Source: Author}
    \label{fig:resnet50_imagenet_inst_precision}
    \end{center}
\end{figure}

For  the first four datasets (instances 1 till 4), we see a drop in precision as more instances are present in images. When dataset images have more object instances, precision values don't seem to correlate with the number of instances. That means that the localization method is not generating relatively more false positives as more object instances are present in the images. Intuitively, the more object instances in an image, the closer they are, and the higher the probability that the localization method will localize groups of instances as a single instance, which will impact recall but not precision. 
...
\bottomrule
\end{tabular}
\caption[Runtime of CAM-based methods for ResNet-50 on ImageNet validation dataset]{Runtime of CAM-ased methods for ResNet-50 on ImageNet validation dataset.  Column-wise maximum is highlighted in bold green.}
...
Each CAM-based method computes a score map as a weighted combination of feature maps in the final convolutional layer. CAM computes as weights the target class related parameters learned in the single fully connected layer, which is a very cheap operation. Grad-CAM++ computes weights from feature map activations and the back-propagated gradients in the final convolutional layer.
...
Using the iterative localization method proposed in section \ref{sec:method_localization_improvement}, we analyze the effect of this method on the ResNet-50 architecture for the synthetic and ImageNet datasets. We use the same models that were used to evaluate the non-iterative approach.

\subsection{Recall versus precision}
The aim of the iterative approach is to improve the accurate localization of ground truth object instances. This localization accuracy is measured by the MaxBoxAccV3 recall metric. 

A naive approach to improving recall would be to use a localization method that exhaustively generates bounding boxes of different granularity at various locations. Such a method would accurately localize most object instances and thus have a high recall value. At the same time this method would suffer from a lot of false positives, i.e., many of the predicted object localization instances would not match with the actual ground truth localization annotations. Hence, precision would be very low. 

To measure the impact of the iterative localization method, we want to measure both precision and recall. Ideally, we would like to have a localization method that has good precision and recall.
...
Due to the number of possible parameter combinations, we have to compare a lot of results. It is rather difficult to represent these results in a single table and draw conclusions from it. 

Therefore we graphically represent and interpret the results as follows:

\begin{itemize}
    \item Per dataset, network and localization method, we create a two-dimensional plot with recall on the horizontal axis and precision on the vertical axis.
    \item For each of the different combinations of mask strategies, merge strategies and stop criteria, an iterative localization experiment is run and the resulting precision and recall are added to the plot.
    \item The precision and recall for non-iterative localization is plotted as two orthogonal lines: A horizontal line for precision and vertical line for recall. We use the precision and recall from the non-iterative approach as baseline to compare with results from the iterative approach.
    \item The horizontal and vertical lines of the baseline create four quadrants which allow easy graphical interpretation of iterative localization results depending in which quadrant a result is situated. Table \ref{tab:iterative_precision_recall_quadrants} visually shows the four quadrants and how to interpret results based on the quadrant the results are plotted in.
\end{itemize}
...
worse recall     & better recall \\ 
\hline
worse precision & worse precision \\
worse recall    & better recall \\ 
\end{tabular}
\caption[Precision and recall quadrants]{Quadrants positioning precision and recall from iterative localization results with respect to baseline localization result.}
\label{tab:iterative_precision_recall_quadrants}
\end{table}

\subsection{Evaluation for ResNet-50 on synthetic datasets} \label{sec:exp_iter_resnet50_syn}
Here we evaluate the results of the iterative localization for the ResNet-50 network on the synthetic test datasets. We only evaluate datasets that have images with a background to analyze the effect of using different mask strategies. The localization methods used are CAM, Grad-CAM++, Score-CAM and MinMaxCAM. The former three methods are evaluated on a model trained without regularization while the latter is evaluated on a model trained with MinMaxCAM regularization. 
